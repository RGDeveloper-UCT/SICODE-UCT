...
from app.services.sicode_ia_contexto import analizar_contexto_usuario, aplicar_contexto_y_nombre


def _orientacion(contexto):
    return analizar_contexto_usuario(contexto)
...
def _aplicar(datos, o):
    ...
    if o.get("titulo_anexo_contexto") and not datos.get("titulo_anexo"):
        datos["titulo_anexo"]=o["titulo_anexo_contexto"]
...
def _crear(resultado,nombre,token,o,usuario_id):
    ...
    a=AnalisisDocumental(usuario_id=usuario_id,tipo_objetivo="LOTE",tipo_detectado="LOTE_DOCUMENTAL",estado="PENDIENTE_VALIDACION",
        paginas_pdf=resultado["paginas_pdf"],paginas_ocr=resultado["paginas_ocr"],metodo_extraccion="SICODE_IA_CONTEXT_FILENAME_OCR",
        ...
        "contexto_interpretado":resultado.get("contexto_interpretado"),"nombre_interpretado":resultado.get("nombre_interpretado")},
...
def procesar_lote_sicode_ia(rutas,nombres,contexto,token,usuario_id):
    ...
    try:
        with app.app_context():
            ...
            _progreso("preparando",2,"Interpretando contexto, nombres de PDF, PyMuPDF, OCR e IA local",usuario_id=usuario_id,lote_token=token)
            for indice,(ruta,nombre) in enumerate(zip(rutas,nombres),start=1):
                ...
                try:
                    ...
                    r=aplicar_contexto_y_nombre(r,nombre,contexto)
            ...
            registrar_bitacora(accion="SICODE_IA_ANALISIS_FONDO",modulo="Coordinación",descripcion=f"SICODE.IA finalizó en segundo plano {len(creados)} PDF; {len(fallidos)} con incidencia.",
                ...
                "fallidos":fallidos,"procesamiento_fondo":True,"lector_pdf":"PyMuPDF","ocr_paralelo":True,"prioridad_contexto_nombre":True},commit=False);db.session.commit()
