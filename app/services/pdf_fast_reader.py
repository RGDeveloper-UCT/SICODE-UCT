...
from app.services.analisis_documental_inteligencia import IAAnalisisNoDisponible, ocr_pagina_multipase, resolver_tesseract
...
def _ocr_una_pagina(ruta, indice, idioma, comando, segunda_pasada):
    ...
    try:
        ...
        try:
            lectura = ocr_pagina_multipase(imagen, idioma=idioma, tesseract_cmd=comando, segunda_pasada=segunda_pasada, timeout=55)
        finally:
            imagen.close()
...
def leer_paginas_rapido(ruta, *, max_paginas=200, ocr_habilitado=True, ocr_idioma="spa", tesseract_cmd=None, ocr_segunda_pasada=False):
    try:
        import pymupdf
    ...
    try:
        if documento.needs_pass and not documento.authenticate(""):
        ...
        for indice in range(total):
            pagina = documento.load_page(indice)
            try: texto = pagina.get_text("text", flags=flags) or ""
            ...
            if utiles < 45: pendientes.append(indice)
    finally:
        documento.close()
    paginas_ocr = 0
    if pendientes and ocr_habilitado:
        ...
        workers = max(1, min(4, int(os.getenv("DOCUMENT_ANALYSIS_OCR_WORKERS", "2")), len(pendientes)))
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="sicode-ocr") as pool:
            futuros = {pool.submit(_ocr_una_pagina, ruta, i, ocr_idioma, comando, ocr_segunda_pasada): i for i in pendientes}
            for futuro in as_completed(futuros):
                indice = futuros[futuro]
                try: _, lectura = futuro.result()
                except (IAAnalisisNoDisponible, Exception):
                    continue
