...
from app.models.coordinacion import (
    ...
    PagoCoordinacion, RegistroCoordinacion, RemisionCoordinacion, RemisionExpediente,
    ReporteMonitoreo,
)
...
class ImportadorCoordinacion:
    ...
    def _ya_importado(self, hoja, fila):
        return RegistroCoordinacion.query.filter_by(
            lote_importacion=self.lote_importacion,
            hoja_origen=hoja,
            fila_origen=fila,
        ).first() is not None
    ...
    def _contabilizar(self, hoja, estado, ya_importado=False):
        info = self.resumen["por_hoja"].setdefault(
            hoja,
            {"total": 0, "completos": 0, "pendientes": 0, "sin_vincular": 0, "ya_importados": 0},
        )
        info["total"] += 1
        self.resumen["total"] += 1
        if ya_importado:
            info["ya_importados"] += 1
            self.resumen["ya_importados"] += 1
        elif estado == "Completo":
            info["completos"] += 1
            self.resumen["completos"] += 1
        elif estado == "Pendiente de vincular":
            info["sin_vincular"] += 1
            self.resumen["sin_vincular"] += 1
        else:
            info["pendientes"] += 1
            self.resumen["pendientes"] += 1
    ...
    def _nuevo_registro(self, tipo, hoja, fila, no_sp=None, rc=None, providencia=None, fecha=None,
        ...
        estado_final = determinar_estado(
            expediente,
            no_sp_norm,
        ...
        return RegistroCoordinacion(
            ...
            rc=self._texto(rc),
            providencia=self._texto(providencia),
            fecha_recepcion=self._fecha(fecha),
            usuario_id=self.usuario_id,
            usuario_origen=self._texto(usuario_origen),
            estado=estado_final,
    ...
    def _procesar_instalaciones(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            if self._ya_importado(hoja, fila):
                self._contabilizar(hoja, None, True)
                continue
            registro = self._nuevo_registro(
                "INSTALACION", hoja, fila, sp, rc, providencia, fecha,
                usuario_origen=usuario,
                campos_clave=[sp, rc, providencia, fecha, usuario],
            )
    ...
    def _procesar_desinstalaciones(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            if self._ya_importado(hoja, fila):
                ...
                continue
            registro = self._nuevo_registro(
                "DESINSTALACION", hoja, fila, sp, rc, providencia, fecha,
                usuario_origen=usuario,
                campos_clave=[sp, rc, providencia, fecha, folios, usuario],
            )
            self._contabilizar(hoja, registro.estado)
            if importar:
                db.session.add(registro)
    ...
    def _procesar_pagos(self, importar):
        hoja = "PAGOS"
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            if not any(v is not None for v in row[1:5]):
                continue
            _, sp, rc, providencia, fecha, folios, pago_desde, pago_hasta, boleta, total, nota = (list(row) + [None] * 11)[:11]
            if self._ya_importado(hoja, fila):
                self._contabilizar(hoja, None, True)
                continue
            desde_fecha = self._fecha(pago_desde)
            hasta_fecha = self._fecha(pago_hasta)
            ...
            periodo = desde_fecha or periodo_texto or hasta_fecha
            total_decimal = self._decimal(total)
            registro = self._nuevo_registro(
                "PAGO", hoja, fila, sp, rc, providencia, fecha,
                observaciones=nota,
                campos_clave=[sp, rc, providencia, fecha, folios, periodo, boleta, total_decimal],
            )
            self._contabilizar(hoja, registro.estado)
            if importar:
                db.session.add(registro)
                ...
                db.session.add(PagoCoordinacion(
                    registro_id=registro.id,
                    folios=self._texto(folios),
                    periodo_desde=desde_fecha,
                    periodo_hasta=hasta_fecha,
                    periodo_texto=periodo_texto,
                    boleta=self._texto(boleta),
                    total=total_decimal,
                ))
    ...
    def _procesar_anexos(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            if self._ya_importado(hoja, fila):
                ...
                continue
            registro = self._nuevo_registro(
                "ANEXO", hoja, fila, sp, rc, providencia, fecha,
                campos_clave=[sp, rc, providencia, tipo, fecha, folios, fecha_escaneado, numero_anexo],
            )
            self._contabilizar(hoja, registro.estado)
            if importar:
                db.session.add(registro)
                ...
                db.session.add(AnexoCoordinacion(
                    registro_id=registro.id,
                    tipo_anexo=self._texto(tipo),
                    folios=self._texto(folios),
                    escaneado=bool(fecha_escaneado),
                    fecha_escaneado=fecha_escaneado,
                    numero_anexo=self._texto(numero_anexo),
                ))
    ...
    def _procesar_monitoreo(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            if self._ya_importado(hoja, fila):
                ...
                continue
            registro = self._nuevo_registro(
                "MONITOREO", hoja, fila, sp, rc, providencia, fecha,
            ...
            if importar:
                ...
                db.session.add(ReporteMonitoreo(
                    registro_id=registro.id,
                    tipo_documento=self._texto(tipo_doc) or "PROVIDENCIA",
                    numero_reporte=self._texto(reporte_no),
                    tipo_evento=self._texto(tipo_evento),
                ))
    ...
    def _procesar_emitidos(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            if self._ya_importado(hoja, fila):
                ...
                continue
            registro = self._nuevo_registro(
                "DOCUMENTO_EMITIDO", hoja, fila, rc=rc, fecha=fecha,
                campos_clave=[numero, rc, descripcion, destino, fecha],
            )
            self._contabilizar(hoja, registro.estado)
            if importar:
                db.session.add(registro)
                ...
                db.session.add(DocumentoEmitido(
                    registro_id=registro.id,
                    numero_documento=self._texto(numero) or f"SIN-NUMERO-{fila}",
                    descripcion=self._texto(descripcion),
                    destino=self._texto(destino),
                ))
    ...
    def _procesar_actividades(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            if self._ya_importado(hoja, fila):
                ...
                continue
            registro = self._nuevo_registro(
                "ACTIVIDAD", hoja, fila, fecha=fecha, usuario_origen=usuario,
                campos_clave=[actividad, tipo, fecha, usuario],
            )
            self._contabilizar(hoja, registro.estado)
            if importar:
                db.session.add(registro)
                ...
                db.session.add(ActividadCoordinacion(
                    registro_id=registro.id,
                    tipo_actividad=self._texto(tipo),
    ...
    def _procesar_remisiones(self, importar):
        ...
        for fila, row in enumerate(self._iter_filas(hoja), start=2):
            ...
            marcado_remitido = self._texto(remitido) and "REMIT" in self._texto(remitido).upper()
            if not marcado_remitido:
                estado = "Pendiente de remisión"
            elif not partes or not self._texto(folios) or not self._texto(anexos) or not self._texto(foliacion):
                estado = "Información pendiente"
            elif any(resolver_expediente(no_sp)[0] is None for no_sp in partes):
            ...
            registro = self._nuevo_registro(
                "REMISION", hoja, fila, estado=estado, observaciones=observacion_legacy
            )
            self._contabilizar(hoja, registro.estado)
            if importar:
                db.session.add(registro)
                ...
                for no_sp in partes:
                    ...
                    db.session.add(RemisionExpediente(
                        ...
                        observaciones=(
                            "Valores combinados conservados en el registro de remisión."
                            if not valores_individuales else None
                        ),
                    ))

    def procesar(self, importar=False):
        procesadores = [
            self._procesar_instalaciones,
            self._procesar_desinstalaciones,
            self._procesar_pagos,
            self._procesar_anexos,
            self._procesar_monitoreo,
            self._procesar_emitidos,
            self._procesar_actividades,
            self._procesar_remisiones,
        ]
        try:
