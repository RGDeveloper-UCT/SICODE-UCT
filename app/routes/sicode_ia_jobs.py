...
sicode_ia_jobs_bp = Blueprint("sicode_ia_jobs", __name__, url_prefix="/coordinacion/analisis-documental/ia/trabajos")
MAX_ARCHIVOS = 100
...
def _cola():
    return Queue(current_app.config.get("SICODE_IA_QUEUE", "sicode_ia"), connection=_redis(), default_timeout=current_app.config.get("SICODE_IA_JOB_TIMEOUT", 3600))
...
def _tamano(archivo):
    pos = archivo.stream.tell(); archivo.stream.seek(0, 2); total = archivo.stream.tell(); archivo.stream.seek(pos); return total
...
def crear():
    ...
    if not archivos or len(archivos) > MAX_ARCHIVOS:
        flash(f"Seleccione entre 1 y {MAX_ARCHIVOS} PDF.", "danger"); return redirect(url_for("sicode_ia.inicio"))
    if any(not a.filename.lower().endswith(".pdf") for a in archivos):
        flash("Todos los archivos deben ser PDF.", "danger"); return redirect(url_for("sicode_ia.inicio"))
    ...
    if total_bytes > max_mb * 1024 * 1024:
        flash(f"La selección pesa {total_bytes/1024/1024:.1f} MB; el límite actual es {max_mb} MB.", "danger"); return redirect(url_for("sicode_ia.inicio"))
    ...
    try:
        for indice, archivo in enumerate(archivos, start=1):
            ...
            try: destino.chmod(0o600)
            except OSError: pass
            rutas.append(str(destino)); nombres.append(archivo.filename[:240])
        job = _cola().enqueue(procesar_lote_sicode_ia, rutas, nombres, contexto, lote_token, current_user.id,
        ...
        job.meta.update({"usuario_id": current_user.id, "lote_token": lote_token, "fase": "en_cola", "porcentaje": 0,
                         "detalle": "Trabajo recibido. Esperando worker de SICODE.IA.", "archivos_total": len(archivos)})
        job.save_meta()
    except Exception:
...
def espera(job_id):
    ...
    except NoSuchJobError: abort(404)
    if not _job_visible(job): abort(403)
...
def estado(job_id):
    ...
    if not _job_visible(job): abort(403)
    estado_obj = job.get_status(refresh=True)
    estado = getattr(estado_obj, "value", str(estado_obj))
    meta = job.get_meta(refresh=True) or {}
    respuesta = {"job_id":job.id,"estado":estado,"fase":meta.get("fase") or estado,
                 "porcentaje":int(meta.get("porcentaje") or (100 if estado=="finished" else 0)),
                 "detalle":meta.get("detalle") or "SICODE.IA está trabajando.","archivo_actual":meta.get("archivo_actual"),
                 "archivo_indice":meta.get("archivo_indice"),"archivos_total":meta.get("archivos_total"),"lote_token":meta.get("lote_token")}
    if estado == "finished":
        resultado = job.result or {}; token = resultado.get("lote_token") or meta.get("lote_token")
